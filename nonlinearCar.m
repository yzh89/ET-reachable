...
Kx = [ x(2);  y(2);  th(2);
      -x(1); -y(1); -th(1)];
sys.Xset = polytope(Hx, Kx);

%%
th0=linspace(-pi,pi,Nd+1)+pi/Nd;
thg=linspace(-pi,pi,Nd+1);
for i=1:Nd
    Ac{i} = [0, 0, -v0*sin(th0(i));
        0, 0,  v0*cos(th0(i));
        0, 0, 0];
end
...
% linear system
for i=1:Nd
    lsys{i} = elltool.linsys.LinSysContinuous(Ac{i}, Bc, uBoundsEllObj);  
end
% time interval
timeVec = [0 10];  
% initial conditions:
x0EllObj_A = [zeros(2,1); pi/4] + ellipsoid(diag([0.01,0.01,0.0001]));
x0EllObj_B= [5*sqrt(2)*ones(2,1); -3/4*pi] + ellipsoid(diag([0.01,0.01,0.0001]));

% initial directions (some random vectors in R^4):
 dirsMat = [eye(3,3),rand(3,6)];
%dirsMat=[[eye(3);zeros(7,3)],rand(10,7)];
%load('result.mat','dirsMat')
% reach set
startI_=find(thg<=pi/4);
startI=startI_(end);
rsObj_A = elltool.reach.ReachContinuous(lsys{startI}, x0EllObj_A, dirsMat, timeVec,...
    'isRegEnabled', true, 'isJustCheck', false, 'regTol', 1e-7);  
grdHypObj_1 = hyperplane([0; 0; 1], thg(startI+1));
grdHypObj_2 = hyperplane([0; 0; -1], -thg(startI));
%rsObj_B = elltool.reach.ReachContinuous(lsys, x0EllObj_B, dirsMat, timeVec,...
%    'isRegEnabled', true, 'isJustCheck', false, 'regTol', 1e-7); 

%project to see reachable set at end time
basisMat = [1 zeros(1,2); 0 1 zeros(1,1); 0 0 1]';  % orthogonal basis of (x1, x2) subspace
